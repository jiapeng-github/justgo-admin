package com.justgo.admin.utils.excel;
...
public class JsGridReportBase
{
  ...
  public void exportToExcel(String title, String creator, TableData tableData, String sFileName)
    throws Exception
  {
    ...
    sFileName = sFileName + sdf.format(date) + ".xls";
    //this.response.setHeader("Content-Disposition", "attachment;filename=".concat(String.valueOf(URLEncoder.encode(sFileName, "UTF-8"))));
    this.response.setHeader("Content-Disposition", "attachment;filename="+new String(sFileName.getBytes("UTF-8"), "iso8859-1"));
  ...
  private HashMap<String, HSSFCellStyle> initStyles(HSSFWorkbook wb)
  {
    ...
    //  HSSFWorkbook src = new HSSFWorkbook(fs);
      ...
      HSSFSheet sheet = null;//src.getSheetAt(0);
...
//    nstyle.setFont(font);
//    nstyle.setFillForegroundColor(HSSFColor.BLUE_GREY.index);
//    nstyle.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
//    nstyle.setFillBackgroundColor(HSSFColor.BLUE_GREY.index);
